...
| 频道（可选） | 日志频道 / 出击报告频道 各 1 个 |
| 报销资格群（可选） | 1+ 个用于报销资格成员校验的群/频道 |
...
### 1.4 准备日志频道 + 出击报告频道（可选，但推荐）

| 频道 | 用途 |
|---|---|
| 日志频道 | 6 类事件卡片：新申请 / 通过 / 拒绝 / 链接已用 / 链接过期 / 密钥使用 / 报销 5 状态 |
| 出击报告频道 | 仅转发申请人提交的"出击报告"文本，归档用 |

每个频道：
1. 新建一个频道（私有即可）
2. 把 Bot 添加为管理员，勾选"Post Messages"
3. 同样不需要预先记 ID，后续在 `/admin` 面板里转发频道任一条消息即可绑定
...
回到 `[💰 报销管理]` → `[🎯 资格列表]` → `[➕ 添加资格群/频道]`：把"报销前必须在的群或频道"任一消息转发给 Bot。
