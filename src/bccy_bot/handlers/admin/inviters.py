"""邀请人管理：列表 / 添加（5 步引导）/ 启停 / 移除（[v1.0.0-beta.3] 简化）。"""
...
# 步骤（v1.0.0-beta.3 起 5 步）：
#  1 - 等待 telegram_user_id (text 或 /skip)
#  2 - 等待 display_name (text)
#  3 - 选目标群组 (callback)
#  4 - 选材料 (multi-select callback)
#  5 - 选审核模式 (callback)
#  6 - 确认 (callback)
...
async def _push_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.callback_query is not None:
        try:
            await update.callback_query.edit_message_text(text, reply_markup=inviter_add_confirm_keyboard())
            return
        except Exception:  # noqa: BLE001
            pass
    if update.effective_message is not None:
        await update.effective_message.reply_text(text, reply_markup=inviter_add_confirm_keyboard())
...
async def on_add_pick_group(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async with session_scope(context) as session:
        ...
        if g is None or not g.is_active:
            ...
            return
        group_name = g.name
    update_awaiting_data(context, update.effective_user.id, group_id=gid, group_name=group_name, step=4)
...
async def on_add_set_mode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if raw == "_show":
        if not state["data"].get("materials"):
            ...
            return
        update_awaiting_data(context, update.effective_user.id, step=5)
    ...
    update_awaiting_data(context, update.effective_user.id, review_mode=mode, step=6)
...
async def on_add_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state is None or state.get("kind") != AWAIT_KIND:
        return
    d = state["data"]
    required = ["display_name", "group_id", "materials", "review_mode"]
